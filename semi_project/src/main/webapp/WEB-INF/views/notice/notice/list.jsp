...
function orderChange() {
	location.href="${pageContext.request.contextPath}/notice/write.do";
}
</script>
</head>
<body>
	...
	<div class="body-container" style="width: 700px;">
		...
		<table class="table">
			<tr>
				...
				<td align="right">
					<select name="order" class="order-select" onchange="orderChange()">
							<option value="rate" ${condition=="all" ?"selected='selected'":"" }>최신순</option>
							<option value="hitCount" ${condition=="userName" ?"selected='selected'":"" }>조회수순</option>
							<option value="like" ${condition=="reg_date" ?"selected='selected'":"" }>좋아요순</option>
					</select>
				</td>
			</tr>
		</table>
		...
		<table class="table table-border table-list">
			...
			<tbody>
				...
				<tr>
					...
					<td class="left">
						<a href="${articleUrl}&num=${dto.num}">${dto.subject}</a>
						<c:if test="${dto.gap<1}"><img src="${pageContext.request.contextPath}/notice/images/new.gif"></c:if>
					</td> 
					<td>${dto.userName}</td>
					<td>${dto.reg_date}</td>
					<td>${dto.hitCount}</td>
